    ...
    <div class="row">
        <div class="col-md-12 text-secondary font-weight-bold mb-2">
          ...
          <b-card v-if="patient!=null">
            <div class="row">
              <div class="col-md-10">
                  <div v-if="currentDate !=undefined">Date: {{currentDate.toLocaleDateString()}}</div>
                <!-- <b-card-title>Patient Info #{{patient.pid}}</b-card-title> -->
                <div v-if="patient.id>0">
                    <div class="row">
                        <div class="col-md-6">Patient Name: {{patient.fullName}}</div>
                         <div class="col-md-3" v-if="patient.age"> Age: {{patient.age}}</div>
                        <div class="col-md-2" v-if="patient.gender"> Sex: {{patient.gender}}</div>
              ...
              <div class="col-md-2">
                    <div>
                       <span class="w-50 d-inline-block"> NCH:</span> <Status :data="!patient.registration"/>
                    </div>
                   <div>
                         <span class="w-50 d-inline-block">CH-GB:</span> <Status :data="isGb()"/>
                    </div>

                    <div>
                         <span class="w-50 d-inline-block">CH-NGB:</span> <Status :data="patient.registration && (!isGb())"/>
                    </div>
